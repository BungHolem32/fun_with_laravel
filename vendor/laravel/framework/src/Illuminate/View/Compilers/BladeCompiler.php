...
class BladeCompiler extends Compiler implements CompilerInterface {
     ...
     * @return mixed
     */
    protected function compileLanguages($value)
    {
        ...
        {
            if (method_exists($this, $method = 'preCompile'.ucfirst($match[1])))
            {
                $match[0] = $this->$method(array_get($match, 3));
            }
            return isset($match[3]) ? $match[0] : $match[0].$match[2];
        };
        preg_replace_callback('/\B@(\w+)([ \t]*)(\( ( (?>[^()]+) | (?3) )* \))?/x', $callback, $value);
        return $value;
    }
    ...
    protected function preCompileLn($expression)
    {
        $this->langKeys[] = substr($expression, 1, -1);
    }
    protected function compileLn($expression)
    {
        if(!isset($this->langKeysVals))
            $this->langKeysVals = Languages::getTranslation($this->langKeys);


        $key = substr($expression, 1, -1);
        if(isset($this->langKeysVals[$key]) && !empty($this->langKeysVals[$key]))
            return $this->langKeysVals[$key];
        else
            return $key;
    }
     ...
     * @return string
     */
    protected function compileCom($expression)
    {
        if (starts_with($expression, '('))
        {
            /*$expression = substr($expression, 1, -1);
            ...
            {
                $temp2 = $temp[0].'.comIndex';
                $expression = str_replace($temp[0], $temp2, $expression);
                $expression = str_replace('[', "['comName'=>'{$temp[0]}', ", $expression);
