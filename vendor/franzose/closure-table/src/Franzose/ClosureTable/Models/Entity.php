...
class Entity extends Eloquent implements EntityInterface
{
    ...
    protected function isMongo($key){
        if( isset($this->runTimeChildes[$key]) && is_object($this->runTimeChildes[$key]) && get_class($this->runTimeChildes[$key]) == 'App\mongo')
            return true;
        return false;
    }

    protected function isMongoObj($obj){
        if( isset($obj) && is_object($obj) && get_class($obj) == 'App\mongo')
            return true;
        return false;
    }

    protected function isSimple($value){
        if(in_array($value,['object','array']))
            return false;

        return true;
    }

    protected function isTypeSimple($mongo){
        if(isset($mongo->attributes['type']))
            return $this->isSimple($mongo->attributes['type']);
        return false;
    }

    protected function isValueSimple($value){
        return $this->isSimple(gettype($value));
    }

    protected function mongoValue($key){
            if($this->isTypeSimple($this->runTimeChildes[$key]))
                return $this->runTimeChildes[$key]->attributes['value'];

            /*else{
                return $this->runTimeChildes[$key]->getChildren();
            }*/
            else
                return $this->runTimeChildes[$key];
    }

    protected function getChildesKeys($key){
        if($this->hasChildren()){
            $tempChildes =  $this->runTimeChildes[$key]->getChildren();
            foreach($tempChildes as $child){
                $returnArray[] = $child->attributes['key'];
            }
            return $returnArray;
        }
        return array();
