...
class Request extends SymfonyRequest implements ArrayAccess {
    ...
    public function localization(){
        ...
        $loc = \App\Languages::where('active', 1)
            ->where('code', $code)
            ->first();
        if(empty($loc)){
			if(empty(session('local')))
            	$loc = \App\Languages::where('code', 'EN')->firstOrFail();
			else
				$loc = session('local');
            $this->setUrlLang(0);
        }
    ...
    public function setLocal($code){
        if(is_numeric($code))
            $loc = \App\Languages::where('id', $code)->firstOrFail();
        else
            $loc = \App\Languages::where('code', $code)->firstOrFail();
    ...
    public function setUrlLang($param=null){
        if(!empty($param))
            $this->urlLang = true;
        else{
            $this->urlLang = null;
